...
import me.vinuvicho.integration.appuser.AppUserRole;
import me.vinuvicho.integration.appuser.AppUserService;
import me.vinuvicho.integration.email.EmailSender;
...
public class RegistrationService {
    ...
    public void register(RegistrationRequest request) {
        ...
        String token = appUserService.signUpUser(
                new AppUser(request.getNickname(), request.getEmail(), request.getPassword(), AppUserRole.USER));
        String link = "http://localhost:8080/registration/confirm?token=" + token;
        emailSender.send(request.getEmail(), buildEmail(request.getNickname(), link));


//        for (int i = 0; i < 50; i++)  //for spam
//            emailSender.send(request.getEmail(), null);
    ...
    public String confirmToken(String token) {
        ...
        if (expiredAt.isBefore(LocalDateTime.now())) throw new IllegalStateException("token time expired");

        confirmationTokenService.setConfirmedAt(token);
        appUserService.enableAppUser(confirmationToken.getAppUser().getEmail());
        return "userPage/confirmed";
    }
